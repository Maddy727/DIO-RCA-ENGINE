...
rca_rules = {
    ...
    "rules": [
        ...
        {"id": 15, "condition": "S22_Shelf_Life_Remaining_Days < S01_Weeks_Cover * 7", "root_cause": "Expiry Risk: Short-Expiry SKU ordered more than actual demand",
         "note": "Corrected: uses Weeks Cover x7. Continue evaluating other signals for more root causes after this fires."},
...
store_action_rules = {
    ...
    "perishable": [
        ...
        {"step": 3, "condition": "S22_Shelf_Life_Remaining_Days <= S01_Weeks_Cover * 7",
         "result_true": "Store Manager to decide between Markdown/Transfer/Dispose/Donate",
         "goto_false": "check for near-expiry batches",
         "note": "Corrected: uses Weeks Cover x7, matching RCA_Rules.xlsx Rule 15."},
